...
const assets: SenseVoiceAssetManifest = {
  ...
  executablePath: "C:\\tools\\llama-funasr-cli.exe",
  encoderPath: "C:\\models\\sensevoice-small-f16.gguf",
  encoderSha256: "c".repeat(64),
...
test("SenseVoice adapter uses a transient WAV, strips metadata tags, and passes audited assets", async () => {
  ...
  assert.deepEqual(arguments_.slice(0, 7), ["--enc", assets.encoderPath, "-m", assets.modelPath, "--chunk", "6", "-a"]);
  assert.equal(arguments_.includes("--vad"), false);
  assert.equal(arguments_.includes("--language"), false);
  assert.equal(arguments_.includes("zh"), false);
});
...
test("SenseVoice asset audit verifies both manifest paths and SHA-256", async () => {
  ...
  const vad = join(directory, "vad.gguf");
  try {
    await Promise.all([writeFile(executable, "runtime"), writeFile(model, "model"), writeFile(vad, "vad")]);
    const sha256 = (value: string) => createHash("sha256").update(value).digest("hex");
    const encoder = join(directory, "encoder.gguf");
    await writeFile(encoder, "encoder");
    const manifest = {
      ...assets,
      ...
      vadSha256: sha256("vad"),
    };
    await auditSenseVoiceAssets(manifest);
    await assert.rejects(
      () => auditSenseVoiceAssets({ ...manifest, vadSha256: "0".repeat(64) }),
      /sensevoice_asset_hash_mismatch/,
    );
  } finally {
...
test("SenseVoice adapter fails closed for unaudited assets, empty output, and cancellation", async () => {
  ...
  assert.throws(() => validateAssets({ ...assets, transcriptionChunkSeconds: 0 }), /invalid_sensevoice_asset_manifest/);
  const empty = new SenseVoiceCliAsrProvider(assets, async () => ({ stdout: "<|zh|>\n", stderr: "", exitCode: 0 }));
  await assert.rejects(
    () => empty.transcribe(new Uint8Array([0, 0]), "zh", new AbortController().signal),
    /sensevoice_no_speech/,
  );
  const silence = new SenseVoiceCliAsrProvider(assets, async () => ({
    stdout: "<|zh|><|Speech|>\n/sil\n",
    stderr: "",
    exitCode: 0,
  }));
  await assert.rejects(
    () => silence.transcribe(new Uint8Array([0, 0]), "zh", new AbortController().signal),
    /sensevoice_no_speech/,
  );
  const controller = new AbortController();
  controller.abort();
  const provider = new SenseVoiceCliAsrProvider(assets, async () => ({
  ...
  assert.equal(extractTranscript("<|zh|><|Speech|>\n/sil\n"), "");
  assert.equal(extractTranscript("silence\n[no speech]\n"), "");
});
