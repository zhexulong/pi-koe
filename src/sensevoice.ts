...
export class SenseVoiceCliAsrProvider implements AsrProvider {
  ...
  public async transcribe(pcm16: Uint8Array, locale: string, signal: AbortSignal): Promise<string> {
    ...
    try {
      ...
      const result = await this.run(this.assets.executablePath, [
        "-m", this.assets.modelPath,
        "--vad", this.assets.vadPath,
        "-a", audioPath,
        "--language", locale,
      ], signal);
      if (signal.aborted) throw new Error("asr_cancelled");
      if (result.exitCode !== 0) throw new Error("sensevoice_runtime_failed");
      const text = extractTranscript(result.stdout);
      if (text.length === 0) throw new Error("sensevoice_empty_transcript");
      return text;
    } finally {
...
export function validateAssets(assets: SenseVoiceAssetManifest): void {
  if (assets.runtimeId !== SENSEVOICE_RUNTIME_ID
...
export async function auditSenseVoiceAssets(assets: SenseVoiceAssetManifest): Promise<void> {
  validateAssets(assets);
  await Promise.all([access(assets.executablePath), access(assets.modelPath), access(assets.vadPath)]).catch(() => {
    throw new Error("sensevoice_asset_missing");
  });
  const [modelHash, vadHash] = await Promise.all([sha256File(assets.modelPath), sha256File(assets.vadPath)]);
  if (modelHash !== assets.modelSha256.toLowerCase() || vadHash !== assets.vadSha256.toLowerCase()) {
...
function isNonEmptyPath(value: string): boolean { return typeof value === "string" && value.trim().length > 0 && value.length <= 4096; }

function pcm16ToWav(pcm16: Uint8Array): Uint8Array {
  const header = Buffer.alloc(44);
  header.write("RIFF", 0, "ascii"); header.writeUInt32LE(36 + pcm16.byteLength, 4);
  header.write("WAVEfmt ", 8, "ascii"); header.writeUInt32LE(16, 16);
  header.writeUInt16LE(1, 20); header.writeUInt16LE(REQUIRED_PCM_FORMAT.channels, 22);
  ...
  header.writeUInt32LE(byteRate, 28); header.writeUInt16LE(REQUIRED_PCM_FORMAT.channels * 2, 32);
  header.writeUInt16LE(16, 34); header.write("data", 36, "ascii"); header.writeUInt32LE(pcm16.byteLength, 40);
...
function runCommand(executable: string, arguments_: readonly string[], signal: AbortSignal): Promise<CommandResult> {
  return new Promise((resolvePromise, reject) => {
    ...
    const abort = () => { child.kill(); reject(new Error("asr_cancelled")); };
